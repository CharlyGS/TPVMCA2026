...
El presente Proyecto se centra en la configuración y administración de un servidor GNU/Linux basado en **Debian 12 (Bookworm)**, alojado en una máquina virtual.
...
•	Servidor Web: Apache2 con soporte para PHP 8.2, sirviendo un sitio de monitoreo desde una partición dedicada.
...
•	root.tar.gz: Directorio /root, incluyendo los archivos originales del TP y la configuración de claves SSH en la carpeta oculta .ssh.
•	etc.tar.gz: Directorio /etc, con todos los archivos de configuración de los servicios (SSH, Apache, Red, fstab, etc.).
•	opt.tar.gz: Directorio /opt, donde se aloja el script de backup principal (backup_full.sh) y el archivo particion (copia persistente de la información de las particiones del sistema).
...
•	var_parte_aa y var_parte_ab: Archivos que conforman el directorio /var dividido en partes pequeñas para cumplir con los límites de tamaño de GitHub
